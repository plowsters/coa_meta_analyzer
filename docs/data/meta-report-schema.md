...
- `rotation_signature`
- `rotation_guide`
- `leveling_path` through guide build cards
- `warnings`

`projected_dps_index` is a theorycraft index. It is not raw DPS, simulated DPS, observed DPS, or empirical DPS.

`run_config.simulate_rotations` enables M1.11E rotation candidate generation and local APL simulation for selected guide builds. When enabled, `run_config.rotation_duration_ms` controls simulated duration and `run_config.rotation_candidates` caps deterministic candidate variants per build.
...
`rotation_signature` has schema version `coa-rotation-playstyle-v1` and records the compact playstyle shape used for build diversity clustering. It includes core, opener, maintenance, cooldown, and role-tool action keys plus resource-loop, burst-cadence, uptime-mechanic, and range-profile labels.

`rotation_guide` has schema version `coa-rotation-guide-v1` and is the M1.11E guide-ready rotation payload. When `simulate_rotations` is enabled, it is generated by simulating bounded APL candidates, scoring them by role objective, and extracting concise player-facing sections. It contains opener, core loop, priority, cooldown, proc, defensive, healing, support, AoE, movement, action-usage, simulation-summary, reliability, and warning sections. See [Rotation Guide Schema](rotation-guide-schema.md).

Guide build cards include `leveling_path` with schema version `coa-leveling-path-v1`. This payload lists level-by-level AE/TE choices, automatic passive unlocks, deferred essence events, and warnings for the selected build.
