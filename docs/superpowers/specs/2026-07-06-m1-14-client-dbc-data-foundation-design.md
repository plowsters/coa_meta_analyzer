...
| **M1.14E** | mechanics extraction completion | Extract the deferred per-spell mechanical operands M1.14A/C/D left on the db/inferred tiers — cooldown + category cooldown, `SpellRuneCost`, the per-spell effect operands (base points, aura types, amplitudes, misc values, chain targets, coefficient inputs, costs, charges — whether inline in `Spell.dbc` per stock 3.3.5a or in a separate `SpellEffect`-style table, resolved by recon), and the **per-spell GCD base operands D pushed here** (`StartRecoveryTime`/`StartRecoveryCategory`, `damage_class`, GCD attribute bits) — and reconcile them into `coa-mechanics-v1` via the M1.14C per-field precedence engine, so M1.16 has dependable client-sourced operands, not just D's conversion constants. | A, C |
...
| **M1.14G** | live resource-carrier resolution | **Respecified** from a viable/not-viable spike into a runtime-evidence *producer*. Consumes E's `coa-resource-candidate-v1` + `coa-client-spell-closure-v1`; resolves every custom class resource's runtime carrier + behavior (client Lua APIs/events — `UnitAlternativePower*`, `GetComboPoints`, `GetSpellCharges`, `ASCENSION_DISPLAYPOWER` — controlled gameplay traces, UI/frame + WeakAura inspection, memory-bridge/API, with the memory-bridge/API investigation as its *first task*, not its deliverable); emits `coa-resource-observation-v1` then canonical `coa-resource-contract-v1` (per-field authority: verified live > proven current-client static > hypothesis). Needs an **acquisition plan** (one character ≠ 21 classes: class creation/trials, contributors, captured observation bundles, or a controlled test env). **M1.14 cannot close until all load-bearing class resources are resolved**; if exhaustive live coverage does not fit G, a dedicated follow-up submilestone owns it — production work is never hidden inside a "spike". | E (candidate + closure) |
...
is specced and implemented (branch `m1-14-d`, awaiting ff-merge; see
[M1.14D WoW Conversion Primitives](2026-07-17-m1-14-d-wow-constants-design.md)) — it deferred the
per-spell GCD base operands (`StartRecoveryTime`/`StartRecoveryCategory`, `damage_class`, GCD attribute
bits) to **M1.14E**, and class-context resolution plus the HP-regen pair
(`gtRegenHPPerSpt`+`gtOCTRegenHP`) to **M1.16**. **M1.14E** is **specced**
([M1.14E design](2026-07-18-m1-14-e-mechanics-extraction-completion-design.md); real-client recon done —
it carries a **mandatory `Spell.dbc` offset repair** (M1.14A's `power_type`/`school_mask` columns are
wrong, so M1.14C shipped wrong client values) and owns the **class-wide static resource substrate**, with
live carrier resolution pushed to M1.14G); F–G are delineated below. See also the
[client DBC reference](../../data/client-dbc-reference.md) for exact data-source locations.
...
- **B. Client-native CoA attribution and advancement graph** — the `CharacterAdvancement.dbc`
  registry, node-level Builder-oracle parity, and per-record confidence. Extracts the graph and
  proves ownership plus the individually ready legality fields (most legality fields remain
...
M1.14B **extracts** the graph and **proves** ownership plus the individually ready legality fields
(most legality fields remain legitimately unresolved from the stale loose JSON); it does **not**
rewire the legality/tree pipeline to consume the client graph — that staged, per-field Decision 1
supersession, gated on this parity report and semantic-layout validation passing, is **M1.15**'s job
(Decision 21/22).
...
- **Carried-forward M1.14B parity-report diagnostic fix (M1 follow-up).** The shipped
  `coa-builder-parity-v3` report groups its `per_class`/`per_tab` breakdown tables on raw,
  un-canonicalized class labels, so the four CamelCase CoA classes (`WitchDoctor`, `WitchHunter`,
  `KnightOfXoroth`, `SunCleric`) appear as phantom split rows alongside their spaced Builder form.
  The defect is diagnostic-only — the breakdowns feed no readiness or adjudication gate — but it must
  be fixed before M1.14 closes. Group the `per_class`/`per_tab` breakdowns through the same versioned
  `canonical_class_label` (`nfkc-casefold-remove-whitespace-v1`) already used for identity comparison,
